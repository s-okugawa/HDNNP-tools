...
if __name__ == '__main__': 
    ...
    grps=['0.95','0.97','0.99','1.0','1.01','1.03','1.05']
    ...
    plotdir=mdfolder+"result-LC7n/grpplot/"
    traindir=mdfolder+"result-LC7n/traincv/"
    ...
    with open(rstfile, 'w') as rslt:
        ...
        for grp in grps:
            print(f'1000K-LC7n/{grp} data gathering')
            for j in range(1, 11):
                dataname="1000KLC7n-"+grp+"-"+str(j)
                datadir= mdfolder+"1000K-LC7n/"+grp+"/"+str(j)
        ...
        for i in range(1, 11):
            print(f'1000K-LC7n/mix/{i} data gathering')
            for j in range(1, 11):
                dataname="1000KLC7n-mix-"+str(i)+"-"+str(j)
                datadir= mdfolder+"1000K-LC7n/mix/"+str(i)+"/"+str(j)
                logfile= datadir+"/output/CrystalSi64/training.log"
                traincvfile= traindir+dataname+".csv"
                plotfile= traindir+"/plot/"+dataname+".png"
                RTDT=gatherplot(logfile,traincvfile,plotfile,datadir,dataname)
    ...
    #Plotting force/RMSE and TC error of each sample
    allplotfile=plotdir+"alldata-md-LC7n.png"
    fig = plt.figure()
    ax1 = fig.add_subplot(111)
    plt.title("MD 1000K-LC7n & LCmix & gs")
    ...
    #Plotting force/RMSE and TC error of each sample
    allplotfile=plotdir+"allmix-md-LC7n.png"
    fig = plt.figure()
    ax1 = fig.add_subplot(111)
    plt.title("MD 1000K-LC7mix-n")
    ax1.set_xlabel("TC Err (fm 112.1W/m-K:300K)")
    ax1.set_ylabel("force/RMSE (meV/A)")
    ax1.grid(True)
    plt.rcParams["legend.edgecolor"] ='green'
